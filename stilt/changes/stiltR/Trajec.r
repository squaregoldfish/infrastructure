...
if((length(lat)!=length(lon))|(length(lon)!=length(agl))|(length(lat)!=length(agl))){
  stop(paste(format(Sys.time(), "%FT%T"),"ERROR lat, lon, & agl have to be the same length!"))
} #if((length(lat)!=length(lon))|(length(lon)!=length(agl))|(length(lat)!=length(agl))){
...
if(is.null(outname)){
  ...
  else stop(paste(format(Sys.time(), "%FT%T"),"ERROR 'outname' needs to be specified when using multiple starting locations"))
}
...
if(existsr(outname,outpath)){
  returninfo["status"]<-3
  cat(format(Sys.time(), "%FT%T"),"DEBUG Trajec(): found object", outname, " in ", outpath, "; use this.\n")
  return(returninfo)
} #if(existsr(outname,outpath)){
...
if(sum(is.na(match(varsout,r.names)))>0)stop(paste(format(Sys.time(), "%FT%T"),"ERROR wrong names:",varsout[is.na(match(varsout,r.names))]))
...
#  if(! my.file.exists(paste(metlib,mm,sep=""))){
   if(! is.element(mm,list.files(metlib))){
    stop(paste(format(Sys.time(), "%FT%T"),"Trajec(): Metfile ",metlib,mm," not found",sep=""))
  }
}
...
if(! file.exists(paste(rundir, "PARTICLE.DAT",sep='')) ){
  dat<-matrix(nrow=2,ncol=n.col);dimnames(dat)<-list(NULL,varsout);cat(format(Sys.time(), "%FT%T"),"ERROR Trajec(): PARTICLE.DAT not found\n");
...
if(length(unix.shell(paste("if (-e ",rundir,"core) echo 'core dumped'; endif",sep=""),shell="/bin/csh"))>0){
  dat<-matrix(nrow=2,ncol=n.col);dimnames(dat)<-list(NULL,varsout);cat(format(Sys.time(), "%FT%T"),"ERROR Trajec(): core was dumped, don't use");
...
if(as.numeric(unix(paste("cat ", rundir,"PARTICLE.DAT | wc -l",sep=""))) < 2){
  dat<-matrix(nrow=2,ncol=n.col);dimnames(dat)<-list(NULL,varsout);cat(format(Sys.time(), "%FT%T"),"ERROR Trajec(): PARTICLE.DAT too short");
}else{
