...
# --------------------------------------------------------------------------------------------------
   ...
   if (!is.element("package:ncdf4", search())) library("ncdf4") # Load ncdf4 library
   ...
   for(yeari in unique(refdate$year)){
      ...
      for (time.i in unique(timept)) {
         ...
         if(length(ctsel)==1){
           ...
           gph<-ncvar_get(tm3file, varid="gph", start=c(point[ctsel,],1, time.i),count=c(1,1,nlevs,1))
           ...
           if(start.rd[1]+count.rd[1]-1>length(centerlons)){# stepping over dateline, need to read in last then first long. elements 
              ...
              gpha <- ncvar_get(tm3file, varid="gph", start=c(start.rd, 1, time.i),count=c(count.rda,nlevs,1))
              gphb <- ncvar_get(tm3file, varid="gph", start=c(start.rdb, 1, time.i),count=c(count.rdb,nlevs,1))
              ...
              gph <- ncvar_get(tm3file, varid="gph", start=c(start.rd, 1, time.i),count=c(count.rd,nlevs,1))
           }
           ...
           pt3d<-pt3d+matrix(-start.rd+c(1,1,1),nrow=nrow(pt3d),ncol=ncol(pt3d),byrow=TRUE) #offset correction, to extract from small array to be read from ncdf4
