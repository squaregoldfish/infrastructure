imagell<-function(mat,lg=F,zlims=NULL,zoom=NULL,center=c(-72.172,42.536),rays=F,
                  ...
                  bordersTF=TRUE,part=NULL,locations=NULL,dev="X11",type="",res.png=72,map.title=NULL,cex.part=0.2,
		  dev.continue=F,width.png=7,height.png=7){
...
#'dev.continue' device is not close (only for png files)
...
#'width.png'                    #width for png file (in inch)
#'height.png'                   #height for png file (in inch)
...
if((".png"==substring(dev,nchar(dev)-3,nchar(dev)))|(".pdf"==substring(dev,nchar(dev)-3,nchar(dev)))){
  bitmap(dev,res=res.png,width=width.png,height=height.png);print(paste("output in ",dev,sep=""))
  if(dev.continue){par(mfrow=c(1,2))}}
  print("lon.ll")
  print(lon.ll)
...
if(!is.null(map.title))title(map.title)
if(".png"==substring(dev,nchar(dev)-3,nchar(dev))){
  if(!dev.continue){dev.off()}}
  par(old.par)
  return(zlims)
}
