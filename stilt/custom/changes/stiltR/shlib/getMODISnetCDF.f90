...
!---------------------------------------------------------------------------------------------------
   ...
   print*, now_iso
   PRINT '(4a)', now_iso, 'INFO Looking for files in ''', TRIM(dpath), ''''
   DO k=1,nf
...
!      fname = 'VPRM_input_'//TRIM(varname(k))//'_d01_'//YYYY//'.nc'
   !new format for files processed after 2009
      ...
      WRITE (*,'(4a)', ADVANCE='NO') now_iso,'DEBUG Opening file ', TRIM(fname), '... '
   ...
   IF (newf) THEN
   PRINT '(2a)'      , now_iso, 'DEBUG NEW FILE FORMAT'
   ...
   ELSE
   PRINT '(2a)'      , now_iso,'DEBUG OLD FILE FORMAT'
   ...
   PRINT '(2a)'      , now_iso, 'DEBUG Subroutine getMODISnetCDF, VPRM_init: dimensions for static fields:'
   PRINT '(2a,3(1x,i0))', now_iso, 'DEBUG lon, lat, veg.classes   : ', imax, jmax, nveg
...
!-------------------------------------------------------------------------------
   USE common_data
   USE date_sub, ONLY: iday, get_now
   IMPLICIT NONE
   ...
   DO k=1,n
      ...
      IF (ti_file(k) < 1 .OR. ti_file(k) > timax) THEN
         ...
         IF (.NOT. warning_issued) THEN
            PRINT '(/2a)'                , now_iso, 'DEBUG Notice (subroutine getMODISnetCDF): requested reference time not in file.'
            PRINT '(2a,i0,a,i4,4(1x,i2))', now_iso, 'DEBUG Input item ', k, ', requested date is ', date(:,k)
            PRINT '(2a,f5.0,a/)'         , now_iso, 'DEBUG Missing values ', missing, ' assigned at these points.'
...
!      set VEG_FRA to zero outside of nest
      if (newf) THEN
         if ( subill>1 ) VEG_FRA_file(1:(subill-1),:,:) = 0.
         if ( subiur<imax ) VEG_FRA_file((subiur+1):imax,:,:) = 0.
         if ( subjll>1 ) VEG_FRA_file(:,1:(subjll-1),:) = 0.
